...
export function LeadsFiltersBar({ filters, counties }: Props) {
  ...
  const set = useCallback((updates: Partial<LeadsFilters>) => {
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 p-3 mb-4 space-y-3">
      ...
      <div className="flex flex-wrap gap-2 items-center">
        ...
        <input type="date" value={filters.permitDateFrom || ''} onChange={e => set({ permitDateFrom: e.target.value })}
          className="text-sm border border-gray-200 rounded-lg px-2 py-1 focus:outline-none focus:ring-2 focus:ring-blue-500" aria-label="Permit date from" />
        <span className="text-xs text-gray-400">to</span>
        <input type="date" value={filters.permitDateTo || ''} onChange={e => set({ permitDateTo: e.target.value })}
          className="text-sm border border-gray-200 rounded-lg px-2 py-1 focus:outline-none focus:ring-2 focus:ring-blue-500" aria-label="Permit date to" />

        <span className="text-xs text-gray-500 ml-2">First sales:</span>
        <input type="date" value={filters.firstSalesDateFrom || ''} onChange={e => set({ firstSalesDateFrom: e.target.value })}
          className="text-sm border border-gray-200 rounded-lg px-2 py-1 focus:outline-none focus:ring-2 focus:ring-blue-500" aria-label="First sales from" />
        <span className="text-xs text-gray-400">to</span>
        <input type="date" value={filters.firstSalesDateTo || ''} onChange={e => set({ firstSalesDateTo: e.target.value })}
          className="text-sm border border-gray-200 rounded-lg px-2 py-1 focus:outline-none focus:ring-2 focus:ring-blue-500" aria-label="First sales to" />
      </div>

      {/* Toggles */}
      <div className="flex flex-wrap gap-2">
        {[
          { key: 'openingSoon', label: 'Opening soon' },
          { key: 'neverContacted', label: 'Never contacted' },
          { key: 'followUpDue', label: 'Follow-up due' },
          { key: 'starred', label: '⭐ Starred' },
        ].map(({ key, label }) => (
          <button
            key={key}
            onClick={() => set({ [key]: !filters[key as keyof LeadsFilters] })}
            className={`text-xs px-2.5 py-1 rounded-full border transition-colors ${
              filters[key as keyof LeadsFilters]
