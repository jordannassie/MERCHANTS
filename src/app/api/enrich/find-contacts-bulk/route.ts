 ...
 * Run Google Places matching for multiple leads.
 * Body: { leadIds?: string[], mode?: 'selected' | 'hot_missing_phone' }
 * Returns: { results: { leadId, status }[], summary }
 */
...
const MAX_BATCH = 25
const DELAY_MS = 300 // be polite to the Places API

const schema = z.union([
  z.object({
    mode: z.literal('selected'),
    leadIds: z.array(z.string().uuid()).min(1).max(MAX_BATCH),
  }),
  z.object({
    mode: z.literal('hot_missing_phone'),
...
export async function POST(request: NextRequest) {
  ...
  type LeadRow = LeadForSearch & { google_place_id: string | null; enrichment_status: string | null }
  const leads = rawLeads as unknown as LeadRow[]

  const results: Array<{
    leadId: string
    status: 'found' | 'review' | 'not_found' | 'error' | 'skipped'
    phone: string | null
    website: string | null
  ...
  for (const lead of leads) {
    ...
    if (lead.google_place_id && lead.enrichment_status === 'completed') {
    ...
    if (result.status === 'error') {
      await db.from('leads').update({ enrichment_status: 'failed', enrichment_error: result.error ?? null }).eq('id', lead.id)
      results.push({ leadId: lead.id, status: 'error', phone: null, website: null, confidence: null, error: result.error })
    } else if (result.status === 'not_found') {
      await db.from('leads').update({ enrichment_status: 'failed', enrichment_error: 'No match' }).eq('id', lead.id)
    ...
    // Rate limit: be polite to the Places API
